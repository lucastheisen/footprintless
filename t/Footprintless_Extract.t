...
sub file_ok {
    ...
    is(slurp($path), slurp(File::Spec->catfile($bar_dir, @file)),
...

{
    ...
    ok(
        Footprintless::Extract
            ->new(archive => 
                File::Spec->catfile($test_dir, 'data', 'resources', 'bar.tar'))
            ->extract(to => $temp_dir),
        'tar extract');
...

{
    ...
    ok(
        Footprintless::Extract
            ->new(archive => 
                File::Spec->catfile($test_dir, 'data', 'resources', 'bar.tgz'))
            ->extract(to => $temp_dir),
        'tgz extract');
...

{
    ...
    ok(
        Footprintless::Extract
            ->new(archive => 
                File::Spec->catfile($test_dir, 'data', 'resources', 'bar.war'))
            ->extract(to => $temp_dir),
        'zip extract');
