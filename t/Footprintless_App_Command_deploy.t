...
test_deployment($coordinate, 'clean',
    before => sub {
        my ($footprintless) = @_;
        my $webapps_dir = $footprintless->entities()
            ->get_entity("$coordinate.to_dir");
    ...
    validator => sub {
        ...
        ok(!-f File::Spec->catfile($webapps_dir, 'bar.war'), 
            'cleaned bar.war');
    });

test_deployment($coordinate, 'deploy',
    validator => sub {
        my ($footprintless) = @_;
        my $webapps_dir = $footprintless->entities()
            ->get_entity("$coordinate.to_dir");
        ok(-f File::Spec->catfile($webapps_dir, 'bar.war'), 
            'deployed bar.war');
        ok(-f File::Spec->catfile($webapps_dir, 'baz.war'), 
            'deployed baz.war');
    });
