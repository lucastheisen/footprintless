...
sub batch_command {
    ...
    (@commands) = @_;
    $command_options = pop(@commands) if (ref($commands[$#commands]) eq 'Footprintless::Command::CommandOptions');
...
sub cp_command {
    ...
    $source_command_options = shift if (ref($_[0]) eq 'Footprintless::Command::CommandOptions');
    $destination_path = _escape_path(shift);
    $destination_command_options = shift if (ref($_[0]) eq 'Footprintless::Command::CommandOptions');
    %cp_options = @_;
...
sub write_command {
    ...
    $filename = shift;
    @lines = @_;
    $command_options = pop(@lines) if (ref($lines[$#lines]) eq 'Footprintless::Command::CommandOptions');
